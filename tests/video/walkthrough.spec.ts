import { test, expect, type Locator } from '@playwright/test';
import path from 'node:path';

// A single continuous journey, paced to be watched. Records to test-results/;
// `npm run video` copies the clip to recorded_video.webm in the repo root.
// Point it at a deployment with PLAYWRIGHT_BASE_URL, or let it start a local dev
// server (needs GEMINI_API_KEY in .dev.vars for the AI panels to populate).

const SAMPLE = path.resolve('public/samples/service-agreement.pdf');
const account = { name: 'Syed Maaz', email: 'syedmaaz.demo@gmail.com', password: 'syed-maaz-2026' };
const guest = { name: 'Rohan Mehta', comment: 'Looks good — confirming the 40 / 30 / 30 payment split before we sign.' };

test('Clause — end to end', async ({ page, context }) => {
  test.setTimeout(420_000);
  const beat = (ms = 1500) => page.waitForTimeout(ms);
  const read = () => page.waitForTimeout(3600);

  // Types like a person: a short fumble partway through, a pause, backspace, correction.
  const human = async (loc: Locator, text: string) => {
    await loc.click();
    const at = Math.max(3, Math.floor(text.length * 0.5));
    await loc.pressSequentially(text.slice(0, at), { delay: 85 });
    const slip = text.slice(at, at + 2).split('').reverse().join('') || 'x';
    await loc.pressSequentially(slip, { delay: 95 });
    await page.waitForTimeout(450);
    for (let i = 0; i < slip.length; i++) { await loc.press('Backspace'); await page.waitForTimeout(95); }
    await loc.pressSequentially(text.slice(at), { delay: 85 });
  };

  await test.step('Create an account', async () => {
    await page.goto('/');
    await expect(page.getByRole('heading', { name: 'Make room for clarity.' })).toBeVisible();
    await read();
    await human(page.getByLabel('Full name', { exact: true }), account.name);
    await beat(600);
    await human(page.getByLabel('Email address'), account.email);
    await beat(700);
    await page.getByLabel('Password', { exact: true }).pressSequentially(account.password, { delay: 55 });
    await beat(1400);
    await page.getByRole('button', { name: 'Create workspace', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Your document workspace.' })).toBeVisible({ timeout: 25_000 });
    await beat(2800);
  });

  await test.step('Upload a PDF and watch the AI summary land', async () => {
    await page.getByRole('button', { name: 'Upload PDF', exact: true }).click();
    await beat();
    await page.locator('input[type=file]').setInputFiles(SAMPLE);
    await beat(2000);
    await page.getByRole('button', { name: 'Upload & understand' }).click();
    await expect(page.getByRole('heading', { name: 'service-agreement.pdf', exact: true })).toBeVisible();
    await beat();
    await expect(page.locator('.document-card .document-status.ready')).toBeVisible({ timeout: 120_000 });
    await expect(page.locator('.document-card .card-summary p')).not.toContainText('Reading your document');
    await read();
    await read();
  });

  await test.step('Open the document — summary and key facts up top', async () => {
    await page.getByRole('button', { name: 'Open service-agreement.pdf', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'service-agreement', exact: true })).toBeVisible();
    await expect(page.locator('.pdf-paper > canvas')).toBeVisible();
    await expect(page.locator('.overview-summary .markdown')).toBeVisible();
    await read();
    await read();
    await page.locator('.pdf-scroll').hover();
    await page.mouse.wheel(0, 500);
    await beat(1800);
    await page.mouse.wheel(0, 500);
    await beat(1800);
    await page.getByRole('button', { name: 'Next page', exact: true }).click();
    await beat(2800);
    await page.locator('.key-facts button').first().click(); // a key fact jumps the viewer to its page
    await beat(3000);
  });

  await test.step('Ask questions — grounded, streamed, with citations', async () => {
    const box = () => page.getByRole('textbox', { name: 'Ask a question about this PDF' });
    const settle = async () => {
      const answer = page.locator('.chat-message.assistant').last().locator('.markdown');
      await expect(answer).toBeVisible({ timeout: 90_000 });
      let prev = '';
      for (let i = 0; i < 60; i++) {
        await page.waitForTimeout(800);
        const cur = (await answer.textContent()) ?? '';
        if (cur.length > 8 && cur === prev) break;
        prev = cur;
      }
      await read();
    };
    await human(box(), 'What is the total fee, and how is it paid?');
    await beat(700);
    await box().press('Enter');
    await settle();
    await page.locator('.chat-message.assistant .source-chips button').first().click(); // follow the citation
    await beat(3400);
    await box().click();
    await box().pressSequentially('And how much of that is due at signing?', { delay: 45 }); // follow-up — needs memory
    await beat(600);
    await box().press('Enter');
    await settle();
    await box().click();
    await box().pressSequentially('Does the contract include a non-compete clause?', { delay: 45 }); // not in the doc — should decline
    await beat(600);
    await box().press('Enter');
    await settle();
  });

  await test.step('Find a document by meaning, not filename', async () => {
    await page.locator('.viewer-back').click();
    await expect(page.getByRole('heading', { name: 'Your document workspace.' })).toBeVisible();
    await beat(2000);
    await page.getByRole('switch', { name: 'Search by meaning' }).click();
    await beat(800);
    await human(page.getByLabel('Search documents'), 'employment contract terms');
    await expect(page.locator('.document-card')).toContainText('service-agreement.pdf', { timeout: 20_000 });
    await read();
    await page.getByLabel('Clear search').click();
    await beat(1200);
  });

  let shareUrl = '';
  await test.step('Share a link — no account needed', async () => {
    await page.getByRole('button', { name: 'Open service-agreement.pdf', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'service-agreement', exact: true })).toBeVisible();
    await beat(1400);
    await page.getByRole('button', { name: 'Share document', exact: true }).click();
    await beat(1400);
    await human(page.getByLabel('Invitation name'), 'Team review');
    await beat(1200);
    await page.getByRole('button', { name: 'Create secure link', exact: true }).click();
    await expect(page.getByLabel('Copy this link now')).toHaveValue(/\/share#/);
    shareUrl = await page.getByLabel('Copy this link now').inputValue();
    await read();
    await page.keyboard.press('Escape');
  });

  await test.step('Open as a guest and leave a comment', async () => {
    await context.clearCookies();
    await page.goto(shareUrl);
    await expect(page.getByText('Guest access', { exact: true })).toBeVisible();
    await expect(page.locator('.pdf-paper > canvas')).toBeVisible();
    await read();
    await page.getByRole('tab', { name: /Comments/ }).click();
    await beat(1400);
    await human(page.getByRole('textbox', { name: 'Your name for comments' }), guest.name);
    await beat(700);
    await page.getByRole('textbox', { name: 'Write a comment', exact: true }).pressSequentially(guest.comment, { delay: 28 });
    await beat(1400);
    await page.getByRole('button', { name: 'Post', exact: true }).click();
    await expect(page.getByText(`${guest.name} (guest)`, { exact: true })).toBeVisible();
    await read();
  });

  await test.step('Back as the owner — revoke access', async () => {
    await page.goto('/');
    await page.getByRole('button', { name: 'Sign in' }).click();
    await beat(1100);
    await page.getByLabel('Email address').pressSequentially(account.email, { delay: 40 });
    await page.getByLabel('Password', { exact: true }).pressSequentially(account.password, { delay: 40 });
    await beat(800);
    await page.getByRole('button', { name: 'Sign in to your workspace', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Your document workspace.' })).toBeVisible();
    await beat(1600);
    await page.getByRole('button', { name: 'Open service-agreement.pdf', exact: true }).click();
    await page.getByRole('button', { name: 'Share document', exact: true }).click();
    await beat(2000);
    await page.getByRole('button', { name: 'Revoke Team review' }).click();
    await read();
    await page.keyboard.press('Escape');
    await context.clearCookies();
    await page.goto(shareUrl);
    await expect(page.getByRole('heading', { name: /couldn.t open this workspace/i })).toBeVisible();
    await beat(4000);
  });
});
